...
TITLE = "保健室 Infirmary"
OPTIONS = [
    ("徐芃學姊療傷", "inf_senpai_xu_intro", "trust", 3),
    ("休息一下", "inf_rest_intro", "stress", -6),
]

def _build(assets, state):
    state.setdefault("infirmary_sel", 0)
    return {}
...
def _loop(screen, state, assets):
    ...
    sel = int(state.get("infirmary_sel", 0)) % len(OPTIONS)
    running = True
    while running and state.get("current") == "infirmary":
