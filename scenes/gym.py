...
TITLE = "體育館 Gym"
OPTIONS = [
    ("顧子昂訓練（重力）", "gym_guzia_intro", "stress", 4),
    ("王老師特訓", "gym_teacher_wang_intro2", "familiarity", 2),
]

def _build(assets, state):
    state.setdefault("gym_sel", 0)
    return {}
...
def _loop(screen, state, assets):
    ...
    sel = int(state.get("gym_sel", 0)) % len(OPTIONS)
    running = True
    while running and state.get("current") == "gym":
