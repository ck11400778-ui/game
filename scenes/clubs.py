...
TITLE = "社團大樓 Clubs"
OPTIONS = [
    ("音樂社｜高柔瑀", "clubs_gaorouyu_intro", "familiarity", 2),
    ("戲劇社｜李柔彤", "clubs_liroutong_intro", "familiarity", 2),
    ("攝影社｜方子謙", "clubs_fangziqian_intro", "trust", 2),
]

def _build(assets, state):
    state.setdefault("clubs_sel", 0)
    return {}
...
def _loop(screen, state, assets):
    ...
    sel = int(state.get("clubs_sel", 0)) % len(OPTIONS)
    running = True
    while running and state.get("current") == "clubs":
