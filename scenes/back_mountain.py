...
TITLE = "後山 Back Mountain"
OPTIONS = [
    ("與周子齊對話（影子）", "back_zouziqi_intro", "trust", 2),
    ("與阿澤傾訴", "back_aze_intro", "familiarity", 4),
]

def _build(assets, state):
    state.setdefault("back_mountain_sel", 0)
    return {}
...
def _loop(screen, state, assets):
    ...
    sel = int(state.get("back_mountain_sel", 0)) % len(OPTIONS)
    running = True
    while running and state.get("current") == "back_mountain":
