...
TITLE = "圖書館 Library"
OPTIONS = [
    ("與林思妍共讀", "library_linsiyan_intro", "trust", 3),
    ("與黃昱翔討論題目", "library_huangyuxiang_intro", "familiarity", 2),
    ("遇見阿澤", "library_aze_intro", "familiarity", 3),
]

def _build(assets, state):
    state.setdefault("library_sel", 0)
    return {}
...
def _loop(screen, state, assets):
    ...
    sel = int(state.get("library_sel", 0)) % len(OPTIONS)
    running = True
    while running and state.get("current") == "library":
